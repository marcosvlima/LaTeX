...
\section{Leia Está seção Antes de Começar}
...
Com bases nessas alterações sugiu as modificações para o \emph{template} não oficial da Universidade Regional Integrada do Alto Uruguai e das Missões (URI). Para estas alterações foi modificado o arquivo {\ttfamily utfpr-abntex2.cls} pra {\ttfamily uri-abntex2.cls} contendo as modificações necessárias para apresentação de trabalhos acadêmicos da Instituição. 

\subsection{Isso é um teste}
\label{subsec:teste}
...
A compilação para gerar um arquivo no formato pdf, incluindo corretamente as referências bibliográficas, deve ser realizada utilizando o comando \verb|makefile|, disponível na mesma pasta onde está o arquivo principal \verb|uri-tcc.tex|. Caso seja alterado o nome do arquivo \verb|uri-tcc.tex|, deverá ser alterado no arquivo \verb|makefile| também.
